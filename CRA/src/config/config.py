...
def parse_args():
    ...
    parser.add_argument('--image_dir', default=str(PROJECT_ROOT / 'CRA' / 'places'), help='Image path of training input data.')
    parser.add_argument('--mask_template_dir', default=str(PROJECT_ROOT / 'CRA' / 'mask_templates'), help='Mask path of training input data.')
    parser.add_argument('--save_folder', default=str(PROJECT_ROOT / 'ckpt'), help='File path of stored checkpoint file in training.')
    parser.add_argument('--device_target', default='GPU', type=str, choices=['GPU', 'Ascend'],
