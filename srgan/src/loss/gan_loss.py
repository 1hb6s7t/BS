...
class DiscriminatorLoss(nn.Cell):
    ...
    def __init__(self, discriminator, generator):
        ...
        # 预先创建足够大的标签张量
        self.max_batch_size = 32  # 设置一个足够大的批次大小
        self.real_label = self.ones((self.max_batch_size, 1), mstype.float32)
        self.fake_label = self.zeros((self.max_batch_size, 1), mstype.float32)
        self.add = ops.Add()
    ...
    def construct(self, hr_img, lr_img):
        ...
        # 使用预创建的标签并截取所需的批次大小
        real_label = self.real_label[:batch_size]
        fake_label = self.fake_label[:batch_size]
        ...
        d_loss_real = self.adversarial_criterion(real_output, real_label)
        # Let the discriminator realize that the sample is false.
        fake_output = self.discriminator(sr)
        d_loss_fake = self.adversarial_criterion(fake_output, fake_label)
        # 使用MindSpore的操作符Add来替代Python的+操作
        d_loss = self.add(d_loss_fake, d_loss_real)
        return d_loss
...
class GeneratorLoss(nn.Cell):
    ...
    def construct(self, hr_img, lr_img):
        ...
        # adversarialloss
        fake_output = self.discriminator(sr)
        adversarial_loss = self.adversarial_criterion(fake_output, real_label)
        ...
        # 使用MindSpore的操作符来计算最终损失
        percep_term = self.mul(percep_loss, 0.006)
        adv_term = self.mul(adversarial_loss, 1e-3)
        loss_term1 = self.add(percep_term, adv_term)
        g_loss = self.add(loss_term1, l2_loss)
        return g_loss
